...
final class TextInserterTests: XCTestCase {
    ...
    func testInsertWritesTranscriptionBeforePastingAndSchedulesRestore() {
        ...
        let inserter = makeInserter(pasteboard: pasteboard, scheduler: scheduler) { _, _ in
    ...
    func testInsertUsesApplicationActiveWhenTextIsReady() {
        let pasteboard = FakePasteboard(string: "old clipboard")
        let scheduler = CapturingScheduler()
        var activeProcessID: pid_t = 11
        var pasteProcessID: pid_t?
        let inserter = makeInserter(
            pasteboard: pasteboard,
            scheduler: scheduler,
            activeProcessProvider: { activeProcessID }
        ) { _, processID in
            pasteProcessID = processID
        }

        activeProcessID = 42
        inserter.insert(text: "new transcription")

        XCTAssertEqual(pasteProcessID, 42)
        XCTAssertEqual(pasteboard.string(forType: .string), "new transcription")
    }
    ...
    private func makeInserter(
        ...
        activeProcessProvider: @escaping () -> pid_t? = { nil },
        pasteAction: @escaping (CGKeyCode, pid_t?) -> Void = { _, _ in }
    ) -> TextInserter {
        TextInserter(
            pasteKeyCode: 9,
            pasteboardProvider: { pasteboard },
            activeProcessProvider: activeProcessProvider,

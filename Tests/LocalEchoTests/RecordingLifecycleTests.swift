...
final class RecordingLifecycleTests: XCTestCase {
    ...
    func testDiscardCancelledRecordingRemovesPartialFile() throws {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent("local-echo-cancel-\(UUID().uuidString).wav")
    ...
    func testDiscardTrackedPartialRecordingRemovesFileBeforeClearingURL() throws {
        var trackedURL: URL? = FileManager.default.temporaryDirectory.appendingPathComponent("local-echo-cancel-\(UUID().uuidString).wav")

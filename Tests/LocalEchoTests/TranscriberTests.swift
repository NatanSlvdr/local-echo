...
final class TranscriberTests: XCTestCase {
    ...
    func testArgumentsIncludeWhisperPromptAsSingleFollowingArgument() throws {
        let prompt = "  Use punctuation, keep product names like Local-Echo.  "
    ...
    func testArgumentsAlwaysDetectLanguage() throws {
        let args = Transcriber(modelSize: "medium").arguments(
            modelPath: "/models/ggml-medium.bin",
            audioURL: URL(fileURLWithPath: "/tmp/input.wav")
        )
        let flagIndex = try XCTUnwrap(args.firstIndex(of: "-l"))
        XCTAssertEqual(args[flagIndex + 1], "auto")
    }

    func testArgumentsDisableCrossWindowContext() throws {
        let transcriber = Transcriber(modelSize: "large-v3-turbo")
        let args = transcriber.arguments(
            modelPath: "/models/ggml-large-v3-turbo.bin",
            audioURL: URL(fileURLWithPath: "/tmp/input.wav")
        )

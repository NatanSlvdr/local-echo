...
enum AppBundleLaunch {
    ...
    static func findLocalEchoAppBundle() -> URL? {
        if let env = ProcessInfo.processInfo.environment["LOCAL_ECHO_APP"]?.trimmingCharacters(in: .whitespacesAndNewlines), !env.isEmpty {
        ...
        for _ in 0..<10 {
            let candidate = dir.appendingPathComponent("Local-Echo.app", isDirectory: true)
        ...
        let homeApps = home.appendingPathComponent("Applications/Local-Echo.app", isDirectory: true)
        if FileManager.default.fileExists(atPath: homeApps.path) { return homeApps }
        let system = URL(fileURLWithPath: "/Applications/Local-Echo.app", isDirectory: true)
    ...
    static func relaunchThroughAppBundleIfNeeded() -> Bool {
        ...
        guard let appURL = findLocalEchoAppBundle() else { return false }

        let executableURL = appURL.appendingPathComponent("Contents/MacOS/local-echo")
        ...
        do {
            ...
            fputs("Error: could not start Local-Echo.app (open exited with \(launcher.terminationStatus)).\n", stderr)
        } catch {
            fputs("Error: could not start Local-Echo.app: \(error.localizedDescription)\n", stderr)
        }
        return false
    }
}

...
final class AudioCaptureUnit {
    ...
    private let listenerQueue = DispatchQueue(label: "LocalEcho.AudioDeviceChanges")
    ...
    private func configure(route: AudioEngineCacheState.Route) throws {
        ...
        guard let component = AudioComponentFindNext(nil, &description) else {
            throw NSError(domain: "LocalEcho.AudioRecorder", code: 1, userInfo: [
    ...
    func stop() throws {
        ...
        guard renderState.framesWritten > 0 else {
            throw NSError(domain: "LocalEcho.AudioRecorder", code: 2, userInfo: [
        ...
        guard renderState.peakLevel > 0 else {
            throw NSError(domain: "LocalEcho.AudioRecorder", code: 4, userInfo: [
    ...
    private func verifyDevice(_ expected: AudioDeviceID, bus: AudioUnitElement) throws {
        ...
        guard actual == expected else {
            throw NSError(domain: "LocalEcho.AudioRecorder", code: 3, userInfo: [
    ...
    private func close() {
        ...
        if let unit {
            ...
            guard status == noErr else {
                cleanupFailed = true
                print("Audio capture cleanup failed (status: \(status)). Restart Local-Echo.")
                return
            }
